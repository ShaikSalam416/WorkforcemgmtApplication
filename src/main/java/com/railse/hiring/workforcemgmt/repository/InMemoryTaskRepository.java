...
public class InMemoryTaskRepository implements TaskRepository{
    ...
    @Override
    public List<TaskManagement> findByReferenceIdAndReferenceTypeAndStatusNot(Long referenceId, ReferenceType referenceType, TaskStatus status) {
        return taskStore.values().stream()
    ...
    @Override
    public List<TaskManagement> findByAssigneeIdInAndStatusNotAndDueDateBetween(List<Long> assigneeIds, TaskStatus status, LocalDate startDate, LocalDate endDate) {
        return taskStore.values().stream()
                .filter(task -> assigneeIds.contains(task.getAssigneeId()) &&
                        !task.getStatus().equals(status) &&
